...
if(isset($_POST['submit']))
{
   ...
   $query=$dbh->prepare("INSERT INTO tblassignments(Details,ClassId,SubjectId,SessionId,TermId,Topic,SubmissionDateTime) VALUES(:details,:class,:subject,:session,:term,:topic,:submission)");
   $query->execute(array(':details' => $details, ':class' => $class, ':subject' => $subject, ':session' => $session, ':term' => $term, ':topic' => $topic, ':submission' => $submission));
   $err=$query->errorInfo();
   echo $err[2];
   $lastInsertId=$dbh->lastInsertId();
   if($lastInsertId) {
      $msg="Assignment has been successfully published";
      } else {
...
<html lang="en">
    <head>
        ...
        <link href="../plugins/summernote/summernote-lite.css?fft" rel="stylesheet"> 
...
Do ensure you have completely assigned subjects to your classes. If not, do so <a href="add-subjectcombination.php" class="alert-link text-underline">over here</a>. 
</div>
...
<?php $sql = "SELECT id,ClassName from tblclasses where id<16 order by ClassNameNumeric";
...
<div class="form-group">
                                         ...
                                         <div id="subject">
                                                    ...
                                                    <div class="form-group">
                                                       <!-- I haven't put the php insert for this -->
                                                       <label class="control-label">To Be Submitted On: <span class="text-vermillion">*</span></label>
                                                       <input class="form-control" type="datetime-local" placeholder="Click to select date and time" name="submission" required>
                                                          </div>
<div class="form-group">
